...
class GRN(nn.Module):
    ...
    def forward(self, x):
        ...
        Nx = Gx / (Gx.mean(dim=-1, keepdim=True, dtype=torch.float16) + 1e-6)
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        if self.native_rms_norm:
            ...
            x = x.float()
            variance = (x * x).mean(-1, keepdim=True).half()
            x = x / (torch.sqrt(variance) + self.eps).half()
            if self.weight.dtype in [torch.float16, torch.bfloat16]:
                x = x.to(self.weight.dtype)
            x = x * self.weight
...
class AttnProcessor:
    ...
    def __call__(
        ...
        x = torch.matmul(torch.softmax(torch.matmul(query, key.transpose(-1, -2)).float() * 100.0, dim=-1, dtype=torch.float32).half(), value).transpose(1, 2).reshape(2, -1, self.hidden_size)
